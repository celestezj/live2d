...
- **锁定位置**：**左键双击人物**→ 位置锁定、不能拖动（防止误拖走）；再左键双击解除锁定。与穿脱外套一样是 0.5 秒内两次落在人物身上的点击才算。
- **拽动反馈（锁定状态下）**：位置锁定时，**左键按住人物并拖动**，人物会朝**拽的方向**作出反馈，而且**按拽住的位置区分**：
  ...
  - 松手即**平滑回到正常姿势**（拉力按每帧 15% 衰减）。位移与角度成比例，拽得越远幅度越大（头部转动/抬头最大约 20~25°，身体转身最大约 15°），并叠加在现有情绪/待机姿势之上。
...
- **穿脱外套**：**右键双击人物**（两次右键落在人物身上、0.5 秒内且相隔不远）→ 脱掉外套；再右键双击穿回来。切换约 0.5 秒平滑过渡（外套就是 llny 的 `Param2`，默认演示里本来就在自动缓慢穿脱；手动切换后由你接管，自动摆动停止）。所有模式（演示 / `--viewer` / 情绪）都生效。
- **退出**：按 `ESC`（或 `Alt+F4`）。
