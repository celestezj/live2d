...
- **`--control-port PORT` 外部随时控制（含头顶说话框）**：在 `127.0.0.1:PORT` 起一个 TCP 服务，**每行一个 JSON 对象**即可让外部进程（AI 语音输出管线、机器人前端等）随时切换情绪、**在人物头顶显示/隐藏一段说话文字**、或强制嘴巴开合。完整协议规范如下，其它客户端进程照此对接即可：
  ...
  **控制协议规范（每条一个 JSON 对象，键 = 独立"通道"，取值的语义统一为：给值 = 设置、`null` = 清除/复位）**：
  ...
  - 说话框外观：**扁宽的暖米黄圆角方块/胶囊**（#f8efde 系、上浅下深的柔和渐变、非纯白），圆角弧度很大、四角无尖角，**没有指向人物的小三角尾巴**；一圈很细的浅棕褐描边；文字用**常规（非粗）圆润黑体（微软雅黑 Regular）**、深灰近黑、水平垂直居中；下方带非常淡的柔和投影、有轻微浮起感。框**固定最多 3 行、宽大于高**；字数多时**先自动缩小字号**（缩到 ~0.6 倍仍放不下才截断末行加 `…`）——绝不会向上/向下拉长去挡脸。
...
- **`mock_control.py`（控制端测试工具）**：一个交互式 TCP 客户端，连上 `--control-port` 端口后直接输入命令，不用写代码就能快速测试表情和嘴型：
  ...
  ```
  进入后直接输入情绪名（`help` 看全部 16 种）切情绪；`say <文本>` 在头顶显示说话框、`say` / `say null` 隐藏；`emotion null` 复位回平和；`mouth <0..1>` 强制开嘴、`mouth null` 释放嘴；`demo` 自动轮播几个情绪；`quit` 退出。与 AI 管线用的是**同一个协议**（每行一个 JSON 对象），所以测通了这里 = 管线也能通（具体协议见上一条）。
...
```
输入情绪名切表情、`say <文本>` 头顶说话、`say`/`say null` 隐藏说话框、`emotion null` 复位平和、`mouth <0..1>` 强制开嘴、`mouth null` 释放、`demo` 轮播、`quit` 退出；与 AI 管线是**同一个协议**（每行一个 JSON 对象），测通这里 = 管线也能通。
