...
- **窗口与人物大小**：默认窗口是逻辑 `520x720` 按显示器 DPI 缩放后的物理像素（本机 4K/250% 即 `1300x1800`），所以高 DPI 屏上人物不会再显得小；100% 缩放时仍是 `520x720`。想更大：启动时用 `--width` / `--height`（物理像素）；运行中把鼠标放在人物上**滚轮滚动**（或按 `+` / `-`），人物随**整个窗口**等比放大/缩小、头顶脚下都不会被裁。
