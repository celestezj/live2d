...
**效果展示**：透明桌面宠物（音频对口型 + 鼠标交互）：[▶️ 播放演示视频](./demo.mp4)

<video src="https://github.com/user-attachments/assets/643249063-97dadc03-b6b1-4f43-8fd3-9f8b8cb3c0fa" controls></video>
...
python desktop_pet.py --width 800 --height 1200 --x 100 --y 100  # 指定人物尺寸/窗口左上角（物理像素）
python desktop_pet.py --scale 0.6             # 初始窗口内人物占高比例（放大请用滚轮或 +/-，会连窗口一起变大不裁切）
...
python desktop_pet.py --control-port 5000     # 开 TCP 控制口：外部随时切情绪/头顶说话框/嘴
...
- **拽动反馈（锁定状态下）**：位置锁定时，**左键按住人物并拖动**，人物会朝**拽的方向**作出反馈，而且**按拽住的位置区分**：
  ...
  - 拽住**身体**：往左右拽 → 身体**朝向转向**拽的方向。llny 的躯干角度参数与 Cubism 命名习惯是**反**的（实测修正）：`ParamBodyAngleX` 才是**躯干转身**（胸横向横扫、脚保持锚定、胸部透视收窄；作者在 PRPR 配置里也正是把 yaw 映射到它），所以横向拽 = **头领转 + 躯干同向转**：头转向拽的方向（`ParamAngleX`，最大约 15°）+ 躯干转身（`ParamBodyAngleX`，满拖达到模型满量程 ±10°，复刻 Live2DViewer 的胸部转身）；上下拽 → 拉下蹲 / 拉高（`ParamBodyAngleY`，实测 + = 变矮下沉、- = 拉伸）。
  - 拽住**双腿 / 大腿**（scale 1 时窗口下约 55% 起；分区按窗口画布比例现算，滚轮 / `+`/`-` 改窗口尺寸、或调 `--scale`，触发线都跟人物同步移动、不漂移）→ **害羞反应 + 大腿并拢**：不管往哪个方向拽，反应强度随拽动幅度增大——**轻微下蹲**（`ParamBodyAngleY` 取负，整个身体下沉、实测身高矮 40 多像素）+ **大腿到膝盖整段夹紧并拢**（实测 `Param28`"yy" 是腿部专属参数，取负值即并拢，两条腿列从上到下合并成一条、脚踝仍分开）+ **双手微微内收**挡在身前（`Param41` 左臂 / `Param43` 右臂；注意 llny 的手部 rig 只会让手臂在身体两侧摆动，**做不了真正"双手在身前交叉遮挡"的姿势**，这是模型限制）+ **非常害羞的脸**（`Param13` 脸红、`Param58` 鼓脸、眼睛下瞟 + 向旁躲闪、眼皮半闭、微微低头、抿嘴）。注意同为 "yy/zz" 系的 `Param27` 是全身绕脚下旋转（头会横移），不能用来夹腿。
  - **左键双击裙摆/大腿区域**（两次左键落在人物身上、0.5 秒内且相隔不远）→ 立即触发和拽大腿完全相同的害羞反应（下蹲 + 大腿并拢 + 手臂内收 + 害羞脸），**需要先右键单击锁定位置**（和拽腿反馈一样只在锁定时生效）；按住保持、松开缓缓淡出。**触发区是一段区间**（scale 1 时窗口 45%~67%）：从**裙摆到大腿**，**上方胸部不触发、下方小腿及以下不触发**，双击头部也不触发。触发带跟随人物实际大小（滚轮 / `+`/`-` 改窗口尺寸、调 `--scale` 时判定区都不漂移）。注意两腿之间或边缘的半透明区域点不到（命中检测按上一帧 alpha，透明像素不触发）。
  - 松手即**平滑回到正常姿势**：按住时拉力快速跟手（每帧 15%），松开后**放慢到每帧 7%** 缓缓扭回，避免回正瞬间的抖动。位移与角度成比例，拽得越远幅度越大（头部转动/抬头最大约 20~25°，躯干转身满拖达模型满量程 ±10°），并叠加在现有情绪/待机姿势之上。
- **窗口与人物大小**：默认**人物**是逻辑 `520x720` 按显示器 DPI 缩放后的物理像素（本机 4K/250% 即 `1300x1800`），所以高 DPI 屏上人物不会再显得小；100% 缩放时仍是 `520x720`。悬浮窗比人物**再高 ~1/5**（顶部一段透明带是头顶说话框的预留区，人物锚在窗口底部、屏幕位置不变）。想更大：启动时用 `--width` / `--height`（物理像素，指的是人物尺寸）；运行中把鼠标放在人物上**滚轮滚动**（或按 `+` / `-`），人物连同头顶预留带随窗口等比放大/缩小、头顶脚下都不会被裁。
- **缩放**：**滚轮控制人物大小**——鼠标悬停在人物身上（透明区/拖拽中不响应）上下滚动，窗口等比放大/缩小（每次约 `×1.15`），人物像素随之变、**永不裁切**，缩放时窗口中心锚定不飘；**`+` / `-`** 键与滚轮走同一条路径（逐格 `×1.15`），**`0`** 复位回启动窗口尺寸，**`ESC`** 退出。实现是"改窗口物理像素大小"（重建 DIB + 重设 GL 视口 `glViewport`，视口才是 live2d 渲染尺寸的驱动），**不是** `model.SetScale()`。`--scale`（默认 1.0）只是**启动时**定窗口内人物占高比例，运行中恒不变；`--click-through`（点击穿透）下窗口收不到滚轮，属预期——该模式本来就不响应鼠标。
...
- **`--control-port PORT` 外部随时控制（含头顶说话框）**：在 `127.0.0.1:PORT` 起一个 TCP 服务，**每行一个 JSON 对象**即可让外部进程（AI 语音输出管线、机器人前端等）随时切换情绪、**在人物头顶显示/隐藏一段说话文字**、或强制嘴巴开合。完整协议规范如下，其它客户端进程照此对接即可：
  ```python
  import socket
  s = socket.create_connection(("127.0.0.1", 5000))
  s.sendall(b'{"emotion":"开心"}\n')       # 切情绪；未知情绪名被忽略并打印（仍支持全部 16 种）
  s.sendall(b'{"emotion":null}\n')        # 复位回默认 平和
  s.sendall(b'{"say":"你好呀，我是桌宠"}\n')  # 头顶显示说话框，文本一直保持直到显式隐藏
  s.sendall(b'{"say":null}\n')            # 隐藏说话框
  s.sendall(b'{"mouth":0.7}\n')           # 强制嘴开 0.7（0~1）
  s.sendall(b'{"mouth":null}\n')          # 释放嘴，回到待机/对口型的自动嘴
  ```
  命令在一条连接里可连发多条，彼此独立、可组合（见下）；情绪切换同样约 0.25 秒平滑过渡。

  **控制协议规范（每条一个 JSON 对象，键 = 独立"通道"，取值的语义统一为：给值 = 设置、`null` = 清除/复位）**：

  | 键 | 取值 | 语义 |
  |---|---|---|
  | `emotion` | `"开心"` 等 16 种之一（字符串） | 切情绪，平滑交叉淡入 |
  | `emotion` | `null` | 复位回默认情绪 **平和** |
  | `say` | 任意文本（字符串） | 人物头顶显示**说话框**，文字一直保持 |
  | `say` | `null` | **显式隐藏**说话框 |
  | `mouth` | `0.0~1.0`（数字） | 强制嘴巴开到该开度 |
  | `mouth` | `null` | 释放嘴，交回待机 / 对口型自动控制 |

  - **通道彼此独立、都可单独发**：只想换表情就只发 `emotion`；只想让宠物"说"一句话就只发 `say`——**发 `say` 绝不会改动当前情绪**，反之亦然；每条里多个键也允许，如 `{"emotion":"开心","say":"好耶！"}`。
  - **说话框是"粘住"的**：发 `say` 后文字**一直显示在头顶**（不会自己消失、不随音频/情绪变），**必须再发 `{"say":null}` 才隐藏**（空串 `""` 同隐藏）。
  - **`{}`（空对象）是合法但无控制作用**的对象——一个通道都不给，桌宠无事可做；请至少给一个通道。
  - **未知键一律忽略**（打印提示）——旧的客户端往新版本发老字段、新客户端往旧版本发新字段都不会崩，天然向前/向后兼容。
  - **要扩展新信号**（比如将来加"待机姿势/贴纸/动作"）就在对象里**新增一个键**即可，服务端无需改协议——第三方照着这张表追加新行。
  - 单行必须是一个**完整 JSON 对象**（UTF-8），多行请求请拆成多个对象各占一行。`{"mouth":..}` 在 `--lipsync`/`--listen` 对口型激活期间会被音频能量覆盖（对口型拥有嘴），发 `{"mouth":null}` 才交还。
  - 说话框外观：**扁平的暖米黄圆角矩形**（#f8efde 系、上浅下深的柔和渐变、非纯白），**四角是统一的小圆角**（圆角半径约为框高的 8–12%，是扁圆角矩形——**不是胶囊/半圆/椭圆角**），**没有指向人物的小三角尾巴**；一圈很细的浅棕褐描边；文字用**常规（非粗）圆润黑体（微软雅黑 Regular）**、深灰近黑、水平垂直居中；下方带非常淡的柔和投影、有轻微浮起感。框**固定最多 3 行、宽大于高**；字数多时**先自动缩小字号**（缩到 ~0.6 倍仍放不下才截断末行加 `…`）——绝不会向上/向下拉长去挡脸。
  - 说话框位置：气泡主体底部**始终悬在头发最高点上方、留一段空隙**（不贴顶、也不压发际线）。**宠物绝不会为了气泡缩放或移动**——悬浮窗在人物上方**常驻预留一段透明"天空带"**（约人物高度的 1/5，窗口比人物高这么多、人物锚在底部、屏幕尺寸/位置不变），气泡就住在这片天空里：弹出时零扰动、收起时零扰动，人物始终原尺寸原位置。**任何时刻气泡都不会盖住头/脸**。尺寸全部按窗口等比，滚轮/`+/-` 缩放或 DPI 变化时天空带与人物同步放大缩小。若把宠物拖到屏幕最顶端、天空带不够放整句长话，气泡顶部会在屏幕/窗口上缘被裁一瞬（绝不会向下压头）；往下拖一点即可全显示。注意同一 `--x/--y` 下宠物会因窗口变高而比旧版本低 ~1/5 屏幕高度。
...
- **`mock_control.py`（控制端测试工具）**：一个交互式 TCP 客户端，连上 `--control-port` 端口后直接输入命令，不用写代码就能快速测试表情和嘴型：
  ...
  ```
  进入后直接输入情绪名（`help` 看全部 16 种），或 `emotion <名>` 切情绪（如 `emotion 兴奋`）；`say <文本>` 在头顶显示说话框、`say` / `say null` 隐藏；`emotion null` 复位回平和；`mouth <0..1>` 强制开嘴、`mouth null` 释放嘴；`demo` 自动轮播几个情绪；`quit` 退出。与 AI 管线用的是**同一个协议**（每行一个 JSON 对象），所以测通了这里 = 管线也能通（具体协议见上一条）。
...
- **AI 对话 + 说话框（推荐场景）**：AI 语音管线把识别/回复的文本经 `{"say":"..."}` 实时显示在宠物头顶，情绪用 `{"emotion":"..."}` 同步切换、说完发 `{"say":null}` 收框；嘴部交给 `--lipsync`/`--listen` 自动对口（发 `say` 不影响嘴，文本通道与嘴/情绪完全独立）。
...
```
输入情绪名或 `emotion <名>` 切表情、`say <文本>` 头顶说话、`say`/`say null` 隐藏说话框、`emotion null` 复位平和、`mouth <0..1>` 强制开嘴、`mouth null` 释放、`demo` 轮播、`quit` 退出；与 AI 管线是**同一个协议**（每行一个 JSON 对象），测通这里 = 管线也能通。
