...
- **拽动反馈（锁定状态下）**：位置锁定时，**左键按住人物并拖动**，人物会朝**拽的方向**作出反馈，而且**按拽住的位置区分**：
  ...
  - **左键双击裙摆/私处或大腿区域**（两次左键落在人物身上、0.5 秒内且相隔不远）→ 立即触发和拽大腿完全相同的害羞反应（下蹲 + 大腿并拢 + 手臂内收 + 害羞脸），**需要先右键单击锁定位置**（和拽腿反馈一样只在锁定时生效）；按住保持、松开缓缓淡出。**触发区是一段区间**（scale 1 时窗口 45%~67%）：从**裙摆/私处**到**大腿**，**上方胸部不触发、下方小腿及以下不触发**，双击头部也不触发。触发带跟随人物缩放（改 `--scale`/`+`/`-` 时判定区不漂移）。注意两腿之间或边缘的半透明区域点不到（命中检测按上一帧 alpha，透明像素不触发）。
