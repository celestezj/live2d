...
## 环境（必须遵守）
- 运行/验证优先用本地 `voice-asr` 虚拟环境的 python（来自 <https://github.com/celestezj/voice1>，已装 glfw、live2d 0.7.0.4、sounddevice、pyaudiowpatch）或者没有的话再看看本地是否存在 `voice-tts` 虚拟环境的 python（来自 <https://github.com/celestezj/voice0>），也可以使用；别用系统 python。项目本身**无自建环境文件**（无 requirements.txt / venv），若 voice-asr、voice-tts 都不在，落回按 README §3 建一个 Python 3.10+ 环境并 pip 装齐上述依赖。
